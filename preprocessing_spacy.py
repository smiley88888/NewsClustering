...
class spaCyModel:
    ...
    def process(self, doc):
        doc = self.model(doc)
        tokens = [token.lemma_ for token in doc if not token.is_stop and not token.is_quote and not token.is_space and not token.is_bracket and not token.is_currency and not token.is_punct and not token.is_left_punct and not token.is_right_punct]
        # print(tokens)
        str = ""
...
if __name__ == "__main__":
    ...
    documents = [
        ...
        "9 Ways to Get the Most Out of Your Xbox Gift Card",
        "A Comprehensive Guide on How to Add Friends on Roblox Xbox One"
    ]
